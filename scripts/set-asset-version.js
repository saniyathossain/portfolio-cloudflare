 ...
 *  run's input). sw.js is excluded too — it has its own independent content hash (hash-sw.js) for a
 *  different purpose (service worker cache invalidation) and shouldn't feed into this one.
 */
const fs = require("fs");
...
const hash = computeHash(["index.html", "assets/js/boot.js", "sw.js"]);
...
// preload for styles.min.css needs the same version so it points at the file the browser will
// actually request.
